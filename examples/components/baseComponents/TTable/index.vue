      ...
      <div class="toolbar_top">
        ...
        >
          ...
          <column-set
            ...
            v-bind="$attrs"
            :columns="columns"
            @columnSetting="columnSettingHandle"
          />
        </div>
      </div>
    </div>
...
const constantEscape = (value: any, list: any[], key: string | number, label: string | number) => {
  const res = list.find((item: { [x: string]: any }) => {
...
// 列设置
const columnSettingHandle = (columnSet: never[]) => {
...
// 单选抛出事件radioChange
const radioClick = (row: any, index: null) => {
...
// 点击单选框单元格触发事件
const radioChange = (e: { preventDefault: () => void }, row: any, index: any) => {
  e.preventDefault()
...
// 是否显示表格操作按钮
const checkIsShow = (scope: { row: { [s: string]: unknown } | ArrayLike<unknown> }, item: { noshow: any[]; show: { val: string | any[]; key: string | number }; hasPermi: any; field: string | number; isField: string | number }) => {
  let isNoshow = false
  if (item.noshow) {
    // 双重判断
    item.noshow.map((rs: { isShow: string; val: string | any[]; key: string | number }) => {
      rs.isShow =
    ...
    isNoshow = item.noshow.every((key: { isShow: string }) => {
  ...
  let isPermission = item.hasPermi
    ? (btnPremissions.value as any).includes(item.hasPermi)
    : true
  // table页面合计
