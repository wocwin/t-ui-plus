<template>
  <t-layout-page>
    <t-layout-page-item>
      <t-table title="某列render渲染" :table="table" :columns="table.columns" :isShowPagination="false" />
    </t-layout-page-item>
  </t-layout-page>
</template>
...
const table = ref({
  ...
  columns: [
    ...
    {
      prop: 'vhtml', label: '自定义组件渲染',
      renderHeader: () => {
        return (
          <span style="color:red">自定义组件render渲染</span>
        )
      },
      minWidth: '220',
      render: (text) => {
    ...
    },
    {
      ...
      renderHeader: () => {
        return (
          <span style="color:red">render for循环渲染</span>
        )
      },
      minWidth: '180',
      noShowTip: false,
