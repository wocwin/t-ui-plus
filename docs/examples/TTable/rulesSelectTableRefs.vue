<template>
  <t-layout-page>
    <t-layout-page-item>
      <t-table
        title="单元格编辑TSelectTable--获取单个Ref实例"
        :table="table"
      ...
      >
        <template #toolbar>
          ...
          <el-button type="primary" @click="save">另一种table数据</el-button>
          <el-button @click="clearValidate">清除规则</el-button>
          <el-button type="primary" @click="reset">重置表单</el-button>
          <el-button type="primary" @click="resetSelect">重置用户名称数据</el-button>
        </template>
      </t-table>
    </t-layout-page-item>
  </t-layout-page>
...
// 重置用户名称下拉表格数据
const resetSelect = () => {
  console.log("重置用户名称下拉表格数据")
  singleEditTSelectTable.value.resetTselectTable()
}
