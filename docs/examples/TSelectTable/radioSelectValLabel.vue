  ...
  <t-layout-page>
    <t-layout-page-item>
      <t-select-table
        ...
        :radioSelectValLabel="state.radioSelectValLabel"
...
const state = reactive({
  radioSelectValLabel:'压型2222',
  defaultSelectVal:[] as any,
  table: {
    total: 0,
    currentPage: 1,
    data: [],
...
const pageChange = val => {
  ...
  if (val == 2) {
        state.defaultSelectVal = ['YX2222']
        state.radioSelectValLabel = ''
      } else {
        state.defaultSelectVal = []
        state.radioSelectValLabel = '压型2222'
      }
  getData(val)
}
