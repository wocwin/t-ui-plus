<template>
  <t-layout-page>
    ...
    <el-dialog title="弹窗返回数据" width="40%" v-model="dialogSelectEnt" draggable>
...
const formOpts: any = reactive({
  ...
  fieldList: [
    { label: '账号', value: 'account', type: 'input', comp: 'el-input', event: 'account', rules: { required: true, message: '请输入账号', trigger: 'blur' } },
    { label: '密码', value: 'password', type: 'password', comp: 'el-input', bind: { 'show-password': true } },
    { label: '昵称', value: 'name', type: 'input', comp: 'el-input', rules: { required: true, message: '请输入昵称', trigger: 'blur' } },
    { label: '性别', value: 'sex', placeholder: 'TSelect单选', type: 'select-arr', comp: 't-select', bind: { optionSource: sexList.value, valueKey: 'value' } },
    { label: '状态', value: 'status', placeholder: 'TSelect单选', type: 'select-arr', comp: 't-select', bind: { optionSource: statusList, valueKey: 'value' } },
    { label: '爱好', value: 'hobby', placeholder: 'TSelect多选', type: 'select-arr', comp: 't-select', list: 'hobbyList', bind: { multiple: true, optionSource: hobbyList, valueKey: 'value' } },
